...
def get_bytes_compact(content):
    ...
    recipient_address = varint.encode(0) + base58.b58decode_check(
        content["op"]["Transaction"]["recipient_address"][2:]
    )
    enc_amount = varint.encode(int(content["op"]["Transaction"]["amount"]))
    bytes_compact: bytes = (
        enc_fee + enc_expire_period + enc_type_id + recipient_address + enc_amount
    )
...
def create_transaction(
    ...
    transaction["signature"] = sign_transaction(
        transaction["creator_public_key"], content, sender_private_key
    ).decode("utf-8")
...
def sign_transaction(public_key, content, private_key):
    ...
    signature = varint.encode(0) + keypair.secret_key.sign(encoded_data)
